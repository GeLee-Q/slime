...
class UpdateWeightFromTensor:
    ...
    def _start_bucket_communication_async(self, param_infos):
        """
        Stage A: Start async broadcast for a bucket. 
        """
        pp_size = mpu.get_pipeline_model_parallel_world_size()
        ep_size = mpu.get_expert_model_parallel_world_size()
        rank = dist.get_rank()
        ...
        # Start asynchronous broadcast (for PP & EP), but do not wait.
        comm_handles = []
        if pp_size > 1:
        ...
        # Return all objects to be waited on and intermediate results.
        return {
    ...
    def _process_and_send_bucket_async(self, bucket_data):
        """
        Stage B: Wait for communication, process data, and send to SGLang asynchronously.
        """
        comm_handles = bucket_data["comm_handles"]
        params = bucket_data["params"]
