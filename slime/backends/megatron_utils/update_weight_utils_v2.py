...
class UpdateWeightFromTensor:
    ...
    def _start_all_gather_async(self, param_infos_and_params):
        """
        Starts all_gather and returns handles and intermediate state immediately.
        """
        gather_tasks = []
        handles = []
        ...
        # Return handles and all necessary context for processing.
        return handles, gather_tasks

    def _wait_and_process_all_gather(self, handles, gather_tasks):
        """
        Waits for all_gather to complete and processes the results.
        """
    ...
    def _start_bucket_communication_async(self, param_infos):
        """
        Stage A: Starts async BROADCAST for a bucket, returns handles and prepared parameters.
        """
        pp_size = mpu.get_pipeline_model_parallel_world_size()
        ep_size = mpu.get_expert_model_parallel_world_size()
        rank = dist.get_rank()
        ...
        # Note: A sync might be needed here to ensure empty tensors are allocated.
        torch.cuda.synchronize()

        # Start async broadcast (PP & EP) without waiting.
        broadcast_handles = []
        if pp_size > 1:
        ...
        if ep_size > 1:
            for info, param in zip(param_infos, params):
                if ".experts." in info.name:
                    src_rank = (info.src_rank if info.src_rank in dist.get_process_group_ranks(mpu.get_expert_model_parallel_group()) else rank)
                    handle = torch.distributed.broadcast(
        ...
        all_gather_handles, gather_tasks = self._start_all_gather_async(list(zip(param_infos, params)))

        # Return all handles and intermediate products.
        return {
            "all_gather_handles": all_gather_handles,
            "gather_tasks": gather_tasks,
            "param_infos": param_infos,
        }


    def _process_and_send_bucket_async(self, bucket_data):
        """
        Stage B: Waits for communication, processes data, sends to SGLang asynchronously.
        """
        all_gather_handles = bucket_data["all_gather_handles"]
        gather_tasks = bucket_data["gather_tasks"]
        param_infos = bucket_data["param_infos"]

        gathered_params = self._wait_and_process_all_gather(all_gather_handles, gather_tasks)
        ...
        serialized_named_tensors_list = (
            [None] * dist.get_world_size(self._ipc_gather_group) if self._ipc_gather_src == dist.get_rank() else None
        )
        dist.gather_object(
            serialized_tensors,
            object_gather_list=serialized_named_tensors_list,
        ...
        if dist.get_rank() == self._ipc_gather_src:
            kwargs = {
                "serialized_named_tensors": serialized_named_tensors_list,
    ...
    def update_weights(self):
        ...
        # Store Ray remote call references.
        ray_refs = []

        # Start communication for the first bucket to warm up the pipeline.
        current_bucket_data = self._start_bucket_communication_async(self.param_info_buckets[0])
        
        # This loop processes bucket `i` and prefetches bucket `i+1` at the same time.
        for i in tqdm(range(num_buckets), disable=rank != 0, desc="Update weights (pipelined)"):
            if i + 1 < num_buckets:
                next_bucket_data = self._start_bucket_communication_async(self.param_info_buckets[i + 1])

            ref = self._process_and_send_bucket_async(current_bucket_data)
            if ref:
                ray_refs.append(ref)
                
            if i + 1 < num_buckets:
                current_bucket_data = next_bucket_data
